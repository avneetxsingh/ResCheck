...
export function ResultsView({ result, partial, onReset, showActions = true }: ResultsViewProps) {
  ...
  return (
    <div ref={printRef} className="space-y-4">
      {/* ── Hero: the derived figure, the verdict, and the three gates ──── */}
      <Reveal>
        <section className="rounded-xl border border-border bg-card px-5 py-5">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center">
            {settled && <CheckRing tally={tally} />}
      ...
      {/* ── Section nav, above the content it switches ──────────────────── */}
      {settled && (
        <div
      ...
      {settled && tab === "overview" && (
        <div className="space-y-4">
          ...
          {/* What blocks you · the radar · what to fix */}
          <Reveal delay={120}>
                  ...
                  <div className="mt-1">
                    ...
                    {misses.map((m, i) => (
                      <FindingRow key={`srch-${i}`} tag="SRCH" tone="warn">
                        No recruiter search for <span className="font-mono">{m}</span> would
                        surface you.
                      </FindingRow>
                    ))}
                  </div>
      ...
      {settled && result && tab !== "overview" && (
        <section className="rounded-xl border border-border bg-card px-5 py-4">
            ...
            <>
              ...
              {funnel && funnel.signals.length > 0 ? (
                funnel.signals.map((s) => (
                  <div key={s.key} className="border-b border-border/60 py-2.5 last:border-b-0">
          ...
          {tab === "summary" && <SummaryPanel summary={result.summary} metadata={result.metadata} />}
        </section>
      )}
    </div>
  );
